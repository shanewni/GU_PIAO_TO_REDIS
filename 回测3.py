...
class TdxStockBacktest:
    ...
    def _resample_day_to_week(self, df_day: pd.DataFrame) -> pd.DataFrame:
        """将日线合成为标准周线（自然周）"""
        if df_day.empty:
            return pd.DataFrame()
        # 通用周线合成：以周五为索引标签
    ...
    def get_multi_period_data(self, code: str, count: int = 800,
        ...
        result = {
            'day': day_data,
            'week': self._resample_day_to_week(day_data),  # <--- 新增这一行：预先合成周线
    ...
    @staticmethod
    def calculate_three_buy_signals(df_30m: pd.DataFrame, df_5m: pd.DataFrame, df_day: pd.DataFrame, df_week: pd.DataFrame) -> tuple[List[float], List[float]]:
        ...
        for window_end in range(1, total_length + 1):
            ...
            if window_signal[-1] == 1.0:
                ...
                # ================= 新增：周线级别校验（传参极速版防未来函数） =================
                week_cond_pass = False
                if window_signal[-1] == 1.0 and df_week is not None and not df_week.empty and df_day is not None:
                    current_time = df_30m.index[window_end - 1]
                    current_date = current_time.normalize() # 转换为当天的 00:00:00
                    
                    # 1. 找到本周一的日期
                    monday = current_date - pd.Timedelta(days=current_date.weekday())
                    
                    # 2. 截取历史已定型的周线 (上周末之前)
                    historical_week = df_week[df_week.index < monday]
                    
                    # 3. 获取本周以来的动态日线数据并合成为一根临时的周K线
                    current_week_days = df_day[(df_day.index >= monday) & (df_day.index <= current_date)]
                    
                    if not current_week_days.empty:
                        current_week_bar = pd.DataFrame({
                            '开盘价': [current_week_days['开盘价'].iloc[0]],
                            '最高价': [current_week_days['最高价'].max()],
                            '最低价': [current_week_days['最低价'].min()],
                            '收盘价': [current_week_days['收盘价'].iloc[-1]]
                        }, index=[current_date])
                        # 拼成包含当前动态 K 线的最新周线结构
                        dynamic_week_df = pd.concat([historical_week[['开盘价', '最高价', '最低价', '收盘价']], current_week_bar])
                    else:
                        dynamic_week_df = historical_week[['开盘价', '最高价', '最低价', '收盘价']]

                    if len(dynamic_week_df) >= 3:
                        high_w = dynamic_week_df['最高价'].tolist()
                        low_w = dynamic_week_df['最低价'].tolist()
                        
                        frac_w = gupiaojichu.identify_turns(len(high_w), high_w, low_w)
                        top_indices = [j for j, val in enumerate(frac_w) if val == 1.0]
                        bottom_indices = [j for j, val in enumerate(frac_w) if val == -1.0]
                        # 4. 条件：最近的底分型必须在最近的顶分型之后，且现价必须在该顶分型与底分型价格中枢的下方
                        if bottom_indices and top_indices:
                            # 由于我们需要最近的底分型在最近的顶分型之后，我们先找到最后一个底分型索引，再找到所有在该底分型之前的顶分型索引
                            last_bottom_idx = bottom_indices[-1]
                            valid_tops = [j for j in top_indices if j < last_bottom_idx]
                            
                            if valid_tops:
                                last_top_idx = valid_tops[-1]
                                top_high = high_w[last_top_idx]
                                bottom_low = low_w[last_bottom_idx]
                                mid_price = (top_high + bottom_low) / 2.0
                                
                                current_week_idx = len(dynamic_week_df) - 1
                                
                                # 条件1：至少是底分型开始计数的第三根K线 (差值 >= 2)
                                cond1 = (current_week_idx - last_bottom_idx) >= 2
                                # 条件2：现价在下跌笔的一半位置以下
                                cond2 = current_close < mid_price
                                
                                if cond1 and cond2:
                                    week_cond_pass = True
                                    
                if not week_cond_pass:
                    window_signal[-1] = 0.0  # 未通过周线结构校验，取消起爆信号
                # =======================================================================
    ...
    def calculate_mfi(df, n=14):
        ...
        传入整个 df 自动识别字段
        """
        # 自动适配列名：有些数据是 '收盘价'，有些是 'close'
        c = df['收盘价'] if '收盘价' in df.columns else df['close']
        h = df['最高价'] if '最高价' in df.columns else df['high']
        l = df['最低价'] if '最低价' in df.columns else df['low']
        v = df['成交量'] if '成交量' in df.columns else df['vol']

        # 确保数据是浮点数且填充 0
        v = v.fillna(0).astype(float)
        ...
        # 辅助序列：上涨流量和下跌流量
        pos_mf = pd.Series(np.where(typ > typ_shift, mf, 0), index=df.index)
        neg_mf = pd.Series(np.where(typ < typ_shift, mf, 0), index=df.index)
        
        # 计算 N 日滚动和
        sum_pos_mf = pos_mf.rolling(window=n, min_periods=n).sum()
        sum_neg_mf = neg_mf.rolling(window=n, min_periods=n).sum()
        
        # 计算 MFI
        # 加上一个小值 1e-10 避免除以 0
        mfi = 100 - (100 / (1 + (sum_pos_mf / (sum_neg_mf + 1e-10))))
    ...
    def three_buy_strategy(self, day_df: pd.DataFrame, week_df: pd.DataFrame, min30_data: pd.DataFrame, min30_high: List[float], min30_low: List[float], df_5m: pd.DataFrame) -> pd.DataFrame:
            ...
            # --- 2. 预计算基础指标 ---
            buy_signals, stop_loss_list = self.calculate_three_buy_signals(data, df_5m, day_df, week_df)
    ...
    def run_backtest(self, code: str, period: str = '30min', init_cash: float = 100000.0,
        ...
        week_data = multi_data.get('week', pd.DataFrame())  # <--- 新增获取周线数据
        ...
        # 生成策略信号
        data = self.three_buy_strategy(day_data, week_data, min30_data, min30_high, min30_low, df_5m)
